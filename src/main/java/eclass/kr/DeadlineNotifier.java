...
public class DeadlineNotifier {
    ...
    public static void main(String[] args) {
        ...
        List<AlertPublisher.DeadlineItem> items = action(); // welcome to mess
        AlertPublisher.publish(computeStatus(items), items);
    ...
    public static List<AlertPublisher.DeadlineItem> action() {
        List<AlertPublisher.DeadlineItem> deadlineItems = new ArrayList<>();
        ...
        if (!driver.getPageSource().contains(Constants.NO_EVENT_MESSAGE)) {
            ...
            for (WebElement event : driver.findElements(By.className("card"))) {
                ...
                if (!event.findElement(By.tagName("img")).getAttribute("alt").equals("Course event")
                    ...
                    deadlineItems.add(new AlertPublisher.DeadlineItem(nameOfCourse, nameOfEvent, typeOfEvent, timeOfDeadline));
                    count++;
    ...
    private static String computeStatus(List<AlertPublisher.DeadlineItem> items) {
        if (items.isEmpty()) return "green";
        for (AlertPublisher.DeadlineItem item : items) {
            if (item.deadline.contains("🔴")) return "red";
        }
        for (AlertPublisher.DeadlineItem item : items) {
            if (item.deadline.contains("🟡")) return "yellow";
        }
        return "green";
    }
